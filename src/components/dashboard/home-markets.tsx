...
function statusMeta(status: string) {
  if (status === "registration_open") return { label: "Booking Open", tone: "bg-success-600" };
  return { label: "Opening Soon", tone: "bg-gold-500" };
}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          <p className="mt-3 text-sm text-cloud-300">No markets in {city} right now — try another city.</p>
        ) : (
          <div className="mt-3 flex flex-col gap-4">
            {filtered.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="overflow-hidden rounded-[1.25rem] border border-navy-700 bg-navy-900 shadow-[0_10px_30px_rgba(0,0,0,0.35)]"
                >
                  <div className="relative h-44 w-full bg-navy-800">
                    ...
                    <div className="absolute left-3 top-3 flex flex-col items-center rounded-xl border border-white/15 bg-black/70 px-2.5 py-1.5 leading-none">
                      <span className="text-[8px] font-bold uppercase tracking-wide text-gold-400">{badge.month}</span>
                      <span className="text-lg font-bold text-white">{badge.day}</span>
                      <span className="text-[7px] text-cloud-300">{badge.weekday}</span>
                    </div>
                  ...
                  <div className="p-4">
                    ...
                    {(price?.half != null || price?.full != null) && (
                      <>
                        <p className="mt-4 text-[10px] font-medium uppercase tracking-wide text-charcoal-300">
                          Stall Pricing
                        </p>
                        <div className={cn("mt-1.5 grid gap-2", price.half != null && price.full != null ? "grid-cols-2" : "grid-cols-1")}>
                          {price.half != null && (
                            <div className="rounded-xl border border-navy-700 bg-navy-800 px-3 py-2">
                              <p className="text-[9px] text-charcoal-300">Half Stall</p>
                              <p className="text-sm font-bold text-white">{formatPaise(price.half)}</p>
                            </div>
                          )}
                          {price.full != null && (
                            <div className="rounded-xl border border-navy-700 bg-navy-800 px-3 py-2">
                              <p className="text-[9px] text-charcoal-300">Full Stall</p>
                    ...
                    <div className="mt-3.5 grid grid-cols-[1fr_1.35fr] gap-2">
                      <Link
                        ...
                        className="flex h-11 items-center justify-center gap-1 rounded-xl border border-navy-700 bg-navy-800 text-xs font-bold text-white"
                      >
                        View Details <ArrowUpRight className="h-3.5 w-3.5" />
                      </Link>
                      <Link
                        href={`/events/${event.slug}`}
                        className="flex h-11 items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-gold-500 to-[#ff9135] text-xs font-bold text-navy-950"
                      >

...
export function HomeHeader({
  ...
  return (
    <div>
      ...
      <form
        onSubmit={onSubmit}
        className="mt-5 flex items-center gap-2.5 rounded-2xl bg-cloud-100 py-1 pl-4 pr-1.5 shadow-[0_14px_34px_rgba(0,0,0,0.3)]"
      >
        <Search className="h-[18px] w-[18px] shrink-0 text-navy-900/60" />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={t("dashboard.searchPlaceholder")}
          className="h-[52px] w-full min-w-0 bg-transparent text-sm text-navy-950 placeholder:text-charcoal-300 focus:outline-none"
        />
        <button
          type="submit"
          aria-label="Search"
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-gold-500 to-[#ff9135] text-navy-950 shadow-[0_4px_14px_rgba(201,152,47,0.4)] transition-transform active:scale-95"
        >
